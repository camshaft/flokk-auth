...
var scrypt = require('scrypt').passwordHash;
...
module.exports = function(options) {
  return function(app) {
    ...
    app.user(function(req, id, done) {
      api.user(req.get('x-api-url'), id, done);
    });

    app.userByUsername(function(req, email, done) {
      api.userByEmail(req.get('x-api-url'), email, done);
    });

    app.client(function(req, id, done) {
      api.client(req.get('x-api-url'), id, done);
    });

    app.userDecision(function(req, user, client, done) {
...
API.prototype.user = function(root, id, done) {
  this._userSearch(root, {id: id}, done);
};

API.prototype.userByEmail = function(root, email, done) {
  this._userSearch(root, {email: email}, done);
};

API.prototype.userByFacebook = function(root, facebook, accessToken, refreshToken, done) {
  var self = this;
  debug('searching for user by facebook id', facebook.id);
  self._userSearch(root, {facebook: facebook.id}, function(err, user) {
    if (err) return done(err);
    if (user) return done(null, user);

    self.link(root, 'users', function(err, users) {
      if (err) return done(err);
      debug(users);
      ...
      self.submit(users, 'create', {
        ...
        'birthday': Math.floor((new Date(facebook._json.birthday)).getTime() / 1000),
        'locale': facebook.locale,
        'email': facebook._json.email
      }, function(err, res) {
        if (err) return done(err);
        debug(res.body);
...
API.prototype.userByGoogle = function(root, google, identifier, done) {
  var self = this;
  this._userSearch(root, {google: google.id}, function(err, user) {
    if (err) return done(err);
    if (user) return done(null, user);
...
API.prototype._userSearch = function(root, query, done) {
  var self = this;
  self.link(root, 'users', function(err, users) {
    if (err) return done(err);
    debug(users);
    ...
    self.submit(users, 'find', query, function(err, res) {
      if (err) return done(err);
      debug('user search results', res.body);
      // The user has been removed
      if (!res.body.results || !res.body.results.length) return done(null, false);
...
API.prototype.createUser = function(root, user, done) {
  var self = this;
  self.link(root, 'users', function(err, users) {
    if (err) return done(err);
    debug(users);
...
API.prototype.confirmUser = function(root, id, done) {
  this.user(root, id, function(err, user) {
    if (err) return done(err);
    // TODO mark them as confirmed
    done();
  });
};
...
API.prototype.client = function(root, id, done) {
  var self = this;
  self.link(root, 'clients', function(err, clients) {
    if (err) return done(err);
...
API.prototype.link = function(rootUrl, rel, done) {
  var self = this;
  self.createToken(function(err) {
    if (err) return done(err);

    self.root(rootUrl, function(err, root) {
      if (err) return done(err);
      debug(root);
...
API.prototype.root = function(rootUrl, done) {
  this.follow({root: {href: (rootUrl || this._root)}}, 'root', done);
};
  ...
  self.cache.get(href, function(err, value) {
    ...
    superagent
      ...
      .end(function(err, res) {
        debug('follow error', err);
        if (err) return done(err);
        if (res.notFound) return done();
